...
class PIP(nn.Module):
    ...
    def __init__(self, in_ch=3, out_ch=1, mode='train', deepsuper=True):
        ...
        self.stage4d = PDEU3d(512,256,512)     #第一个d注意改
        ...
        #------------------------PDE--------------------------
        self.fcl1 = nn.Conv2d(64, 1, 1)  #生成PDE的一个参数矩阵
        ...
        self.side1 = nn.Conv2d(512, out_ch, 1)  #深度监督
        self.side2 = nn.Conv2d(512, out_ch, 1)  #这里注意改
        ...
        self.outconv = nn.Conv2d(5 * out_ch, out_ch, 1) #这里注意改
    ...
    def forward(self, x):
        ...
        hx3d, _, _, _, _ = self.stage3d(hx4f)   #这里注意改
        hx3dup = _upsample_like(hx3d,hx3)
        ...
        if self.deepsuper:
            ...
            if self.mode == 'train':
                ...
                # return  s4, s4f, s3f, s2f, s1f, torch.sigmoid(d5), torch.sigmoid(d4), torch.sigmoid(d3), torch.sigmoid(d2),  torch.sigmoid(d1), torch.sigmoid(out) 

                # return  s4f, s3f, s2f, s1f, torch.sigmoid(d5), torch.sigmoid(d4), torch.sigmoid(d3), torch.sigmoid(d2),  torch.sigmoid(d1), torch.sigmoid(out) 

                # return  torch.sigmoid(d5), torch.sigmoid(d4), torch.sigmoid(d3), torch.sigmoid(d2),  torch.sigmoid(d1), torch.sigmoid(out)


                return torch.sigmoid(out) 

                # return   (torch.sigmoid(d4), torch.sigmoid(d3), torch.sigmoid(d2),  torch.sigmoid(d1), torch.sigmoid(out))
